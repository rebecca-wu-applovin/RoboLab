...
"""
Hand-toggle episode for the Franka + SharpaWave composite robot.

Holds the 7 Franka arm joints at their current positions while toggling all 22
finger joints between flat hand and power fist every `--toggle-every` steps.
Sanity-checks the composite articulation and hand action path.

Usage:
    $ python examples/run_franka_sharpa_toggle.py --headless
    $ python examples/run_franka_sharpa_toggle.py --task RubiksCubeTask --headless

Output:
    Per-env videos saved to output/run_franka_sharpa_toggle/<task_env>/
"""
...
parser = argparse.ArgumentParser(description="Run Franka+SharpaWave hand-toggle episode.")
...
from robolab.registrations.sharpa_wave.auto_env_registrations_franka_jointpos import (  # noqa: E402
...
def run_hand_toggle_episode(env, env_cfg=None, *, save_videos=True, video_mode="all",
    ...
    try:
        for count in tqdm(range(num_steps)):
            ...
            actions = torch.cat([arm_pose, hand_action], dim=1)
...
def main():
    output_dir = os.path.join(PACKAGE_DIR, "output", "run_franka_sharpa_toggle")
    os.makedirs(output_dir, exist_ok=True)

    tasks = args_cli.task or ["BananaInBowlTask"]
    auto_register_franka_sharpa_envs(task=tasks)
    task_envs = get_envs(task=tasks)
    print(f"Running Franka+SharpaWave hand toggle on {len(task_envs)} environments: {task_envs}")
